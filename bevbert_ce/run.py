...
def run_exp(exp_name: str, exp_config: str, 
    ...
    if 'CMA' in config.MODEL.policy_name and 'r2r' in config.BASE_TASK_CONFIG_PATH:
        config.TASK_CONFIG.DATASET.DATA_PATH = '/datasets/bevbert/data/datasets/R2R_VLNCE_v1-2_preprocessed/{split}/{split}.json.gz'

    config.local_rank = local_rank
    config.freeze()
    os.system("mkdir -p /datasets/bevbert/data/logs/running_log")
    logger.add_filehandler('/datasets/bevbert/data/logs/running_log/'+config.LOG_FILE)
